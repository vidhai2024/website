  ...
  return (
    <section id="technology" ref={sectionRef} className="relative py-12 md:py-20 overflow-hidden pb-16 md:pb-20">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <motion.div
          className="absolute top-1/4 left-0 w-48 md:w-96 h-48 md:h-96 rounded-full"
          style={{
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 relative z-10">
        ...
        >
          <span className="text-xs md:text-sm uppercase tracking-widest text-primary font-medium">Our Programs</span>
          <h2 className="font-display text-3xl md:text-6xl font-bold mt-3 md:mt-4 mb-4 md:mb-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-6">
          {offerings.map((item, index) => (
            <motion.div
              key={item.id}
              className="group relative rounded-xl md:rounded-2xl bg-card/30 border border-border overflow-hidden cursor-pointer"
            ...
            >
              ...
              {/* Glow effect */}
              <motion.div
                className="absolute -inset-px rounded-xl md:rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500"
                style={{
              ...
              <div className="relative z-10 p-4 md:p-8">
                {/* Icon */}
                <motion.div
                  className="w-10 h-10 md:w-14 md:h-14 rounded-lg md:rounded-xl flex items-center justify-center mb-3 md:mb-6 transition-all duration-300"
                  style={{
                ...
                <div className="mb-4 md:mb-6">
                  ...
                  <h3 className="font-display text-sm md:text-xl font-semibold mt-1 text-foreground">
                    {item.title}
                  </h3>
                  <p className="text-muted-foreground mt-2 md:mt-3 text-xs md:text-sm leading-relaxed hidden md:block">
                    {item.description}
                  </p>
                </div>

                {/* Features - Hidden on mobile */}
                <div className="hidden md:grid grid-cols-2 gap-4 pt-6 border-t border-border/50">
                ...
                {/* Mobile single stat */}
                <div className="md:hidden pt-3 border-t border-border/50">
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-muted-foreground">{item.features[0].label}</span>
                    <span className="text-sm font-bold" style={{ color: item.color }}>{item.features[0].value}</span>
                  </div>
                </div>

                {/* Hover expand indicator */}
                <motion.div
                  className="absolute bottom-3 right-3 md:bottom-4 md:right-4 w-6 h-6 md:w-8 md:h-8 rounded-full flex items-center justify-center"
